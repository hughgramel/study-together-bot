/**
 * Stats Chart Light Mode - Bar chart visualization with light theme
 *
 * Light mode version of StatsChart with white background and improved contrast.
 *
 * @module components/StatsChartLight
 */
...
interface StatsChartLightProps {
  username: string;
  avatarUrl?: string;
...
export const StatsChartLight: React.FC<StatsChartLightProps> = ({
  username,
  avatarUrl,
  metric,
  timeframe,
  ...
  return (
    <div className="w-[1200px] h-[700px] bg-white flex flex-col p-8">
      ...
      <div className="flex items-center gap-6 mb-10">
        ...
        {/* Separator - taller */}
        <div className="w-px h-10 bg-[#D7D7D7]"></div>
          ...
          {[4, 3, 2, 1, 0].map((i) => {
            ...
            return (
              <div key={i} className="flex items-center justify-end">
                <span className="text-[#3C3C3C] text-base font-semibold">
          ...
          {data.map((point, index) => {
            ...
            return (
              <div key={index} className="flex-1 flex flex-col items-center gap-3 group">
                ...
                {/* Label - blue for current period, dark gray for others */}
                <span className={`text-xl font-bold mb-4 ${
                  isHighlighted ? 'text-[#1CB0F6]' : 'text-[#3C3C3C]'
                }`}>
