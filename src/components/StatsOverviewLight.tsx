/**
 * Stats Overview Light Mode - Detailed breakdown with Duolingo-inspired styling
 *
 * Light mode version with white backgrounds and vibrant accents.
 *
 * @module components/StatsOverviewLight
 */

import React from 'react';
import { Flame, Clock, Zap, BookOpen, TrendingUp } from 'lucide-react';

interface StatsOverviewLightProps {
  username: string;
  avatarUrl?: string;
...
export const StatsOverviewLight: React.FC<StatsOverviewLightProps> = ({
  username,
  avatarUrl,
  metric,
  timeframe,
  ...
  /**
   * Gets configuration for the selected metric
   */
  const getMetricInfo = () => {
    switch (metric) {
      case 'hours':
  ...
  return (
    <div className="w-[700px] h-[700px] bg-white flex flex-col p-10">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          {avatarUrl ? (
            ...
            <div className="w-14 h-14 rounded-full bg-[#F7F7F7] border-2 border-[#E5E5E5] flex items-center justify-center text-2xl">
              👤
            </div>
          )}
          <div className="flex flex-col">
            <h2 className="text-[#3C3C3C] text-xl font-bold truncate max-w-[400px]">
              {username}
            </h2>
      ...
      <div className="flex-1">
        <h3 className="text-[#3C3C3C] text-lg font-bold mb-4">{metricInfo.emoji} {metricInfo.title}</h3>
            ...
            return (
              <div
                ...
                className={`rounded-xl p-5 border-2 flex flex-col ${
                  isHighlighted
                    ? 'bg-[#E8F4FD] border-[#1CB0F6]'
                    : 'bg-white border-[#E5E5E5]'
                }`}
              >
                <div className={`text-sm font-semibold uppercase tracking-wide mb-2 ${
                  isHighlighted ? 'text-[#1CB0F6]' : 'text-[#777777]'
                }`}>
                  {item.label}
                </div>
                <div className="text-4xl font-extrabold leading-none text-[#3C3C3C]">
